...
const destinations = [
  { label: "Course Overview Video", href: "/" },
  { label: "Lesson 1 Video", href: `/workshops/enterprise-ai-foundations/${part1Lesson1.slug}` },
  { label: "Lesson 2 Video", href: `/workshops/enterprise-ai-foundations/${part1Lesson2.slug}` },
  { label: "Lesson 1 Summary", href: "/dev/current-work?view=lesson-1-summary" },
  { label: "Lesson 2 Summary", href: "/dev/current-work?view=lesson-2-summary" },
  { label: "Lesson 3 Summary", href: "/dev/current-work?view=lesson-3-summary" },
  { label: "Part 1 Assessment", href: "/workshops/enterprise-ai-foundations/part-1-assessment" },
  { label: "Certificate", href: "/dev/current-work?view=certificate" },
  { label: "Certificate Verification", href: "/verify-certificate" },
  { label: "Learning Journey", href: "/workshops/enterprise-ai-foundations" },
  { label: "Home", href: "/" },
] as const;
...
export default async function CurrentWorkPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  return <main className="assessment-page"><section className="assessment-card dev-current-work"><p className="eyebrow">Development only</p><h1>Current Work</h1><p>Direct access to the Part 1 lesson-completion and assessment work currently under construction.</p><nav aria-label="Current development work"><ul>{destinations.map((destination) => <li key={destination.href}><Link href={destination.href}>{destination.label}<span aria-hidden="true">→</span></Link></li>)}</ul></nav></section></main>;
}
